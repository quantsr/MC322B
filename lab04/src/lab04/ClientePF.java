...
import java.util.Calendar;//substituião de funções ultrapassadas de java.util.Date
...
public class ClientePF extends Cliente {
	...
	public double calculaScore() {
		...
		birthday.setTime(dataNascimento);
		
		int year_birthday = birthday.get(Calendar.YEAR);
		int month_birthday = birthday.get(Calendar.MONTH) + 1;
		int day_birthday = birthday.get(Calendar.DAY_OF_MONTH);
		
		int year_today = currentDay.get(Calendar.YEAR);
		int month_today = currentDay.get(Calendar.MONTH) + 1;
		int day_today = currentDay.get(Calendar.DAY_OF_MONTH);
		
		int age;
		if(month_birthday >= month_today && day_birthday >= day_today) {
			age = year_today - year_birthday;
		}else {
			age = year_today - year_birthday - 1;
		}
		
		//calculo de fator idade
		...
		return CalcSeguro.VALOR_BASE.value * fator_idade * this.getListaVeiculo().size();
	}
