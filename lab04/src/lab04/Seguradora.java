...
public class Seguradora {
	...
	public void calcularPrecoSeguroCliente() {
		for(int i = 0; i < this.listaClientes.size(); i++) {
			int nSinistro = 0;
			for(int j = 0; j < this.listaSinistros.size(); j++) {
				if(this.listaSinistros.get(j).getCliente().getNome().toLowerCase().equals(this.listaClientes.get(i).getNome().toLowerCase())) {
					nSinistro++;
				}
			}
			//calcula valorSeguro do cliente
			double valorCalculadoSeguro = this.listaClientes.get(i).calculaScore() * (1 + nSinistro);
			this.listaClientes.get(i).setValorSeguro(valorCalculadoSeguro);
		}
	}
	...
	public void calcularReceita() {
		double somaSeguro = 0;
		for(Cliente cliente : this.listaClientes) {
			somaSeguro = somaSeguro + cliente.getValorSeguro();
		}
		System.out.println("O balanco de seguro de todos os clientes da seguradora "+ this.getNome()+ " é de: "+ somaSeguro);
	}
