...
public class Main {
	...
	public static Cliente cadastroCliente(Scanner scan) {
		...
		try {
				...
				String  data, cpf, cnpj,  anoFabricacao;
				...
				case "cnpj":
					...
					System.out.println("Digite a data de licenca do cliente: \"dd/MM/yyyy\"");
					data = scan.nextLine();
					dataLicenca = dateFor.parse(data);
					clienteCNPJ.setDataLicenca(dataLicenca);
					//cadastro de veiculo
					cadastroVeiculo = true;
					while(cadastroVeiculo) {
	...
	public static void removercliente(Scanner scan, Seguradora seguradora) {
		...
		while(!end) {
			...
			System.out.println("Digite o nome do cliente a ser removido. Digite 0 para voltar ao menu anterior.");
